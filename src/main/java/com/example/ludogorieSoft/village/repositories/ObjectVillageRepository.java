...
public interface ObjectVillageRepository extends JpaRepository<ObjectVillage, Long> {
    ...
    @Query("SELECT ov FROM ObjectVillage ov WHERE ov.village.id = :villageId AND ov.villageStatus = :villageStatus")
    List<ObjectVillage> findByVillageIdAndVillageStatus(
