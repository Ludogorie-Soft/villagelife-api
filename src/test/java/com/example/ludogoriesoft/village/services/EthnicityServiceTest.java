...
class EthnicityServiceTest {
    ...
    void testFindEthnicityByNameNonExistingEthnicityThrowsApiRequestException() {
        ...
        assertThrows(ApiRequestException.class, () -> ethnicityService.findEthnicityByName("Non-existing Ethnicity"));
    }
    ...
    void testUpdateEthnicityWhenNonExistingEthnicity() {
        ...
        Assertions.assertThrows(ApiRequestException.class, () -> ethnicityService.updateEthnicity(id, ethnicity));
    ...
    void testFindEthnicityByNameWhenNonExistingEthnicity() {
        ...
        Assertions.assertThrows(ApiRequestException.class, () -> ethnicityService.findEthnicityByName(name));
    ...
    void testCreateEthnicity() {
        String newEthnicityName = "New Ethnicity";
        EthnicityDTO ethnicityDTO = new EthnicityDTO();
        ethnicityDTO.setEthnicityName(newEthnicityName);

        when(ethnicityRepository.existsByEthnicityName(newEthnicityName)).thenReturn(false);
        when(ethnicityRepository.save(any(Ethnicity.class))).thenAnswer(invocation -> invocation.getArgument(0));

        EthnicityDTO result = ethnicityService.createEthnicity(ethnicityDTO);

        Assertions.assertNotNull(result);
        assertEquals(newEthnicityName, result.getEthnicityName());
    ...
    @Test
    void testCreateEthnicityWithBlankEthnicityName() {
        EthnicityDTO ethnicityDTO = new EthnicityDTO();
        ethnicityDTO.setEthnicityName("");

        Assertions.assertThrows(ApiRequestException.class, () -> ethnicityService.createEthnicity(ethnicityDTO));

        verify(ethnicityRepository, never()).existsByEthnicityName(anyString());
        verify(ethnicityRepository, never()).save(any(Ethnicity.class));
    }

    @Test
    void testCreateEthnicityWithExistingEthnicityName() {
        String existingEthnicityName = "Existing Ethnicity";
        EthnicityDTO ethnicityDTO = new EthnicityDTO();
        ethnicityDTO.setEthnicityName(existingEthnicityName);

        when(ethnicityRepository.existsByEthnicityName(existingEthnicityName)).thenReturn(true);

        Assertions.assertThrows(ApiRequestException.class, () -> ethnicityService.createEthnicity(ethnicityDTO));

        verify(ethnicityRepository, times(1)).existsByEthnicityName(existingEthnicityName);
        verify(ethnicityRepository, never()).save(any(Ethnicity.class));
    }

    @Test
    void testCreateEthnicityWithValidData() {
        EthnicityDTO ethnicityDTO = new EthnicityDTO();
        ethnicityDTO.setEthnicityName("New Ethnicity");

        when(ethnicityRepository.existsByEthnicityName(anyString())).thenReturn(false);

        EthnicityDTO result = ethnicityService.createEthnicity(ethnicityDTO);

        Assertions.assertNotNull(result);
        Assertions.assertEquals(ethnicityDTO.getEthnicityName(), result.getEthnicityName());

        verify(ethnicityRepository, times(1)).existsByEthnicityName(ethnicityDTO.getEthnicityName());
        verify(ethnicityRepository, times(1)).save(any(Ethnicity.class));
    }

    @Test
    void testUpdateEthnicityWithBlankEthnicityName() {
        Long ethnicityId = 1L;
        EthnicityDTO ethnicityDTO = new EthnicityDTO();
        ethnicityDTO.setEthnicityName("");

        Assertions.assertThrows(ApiRequestException.class, () -> ethnicityService.updateEthnicity(ethnicityId, ethnicityDTO));
    ...
    @Test
    void testUpdateEthnicityWithExistingEthnicityName() {
        Long ethnicityId = 1L;
        String existingEthnicityName = "Existing Ethnicity";
        EthnicityDTO ethnicityDTO = new EthnicityDTO();
        ethnicityDTO.setEthnicityName(existingEthnicityName);
        ...
        Ethnicity existingEthnicity = new Ethnicity();
        existingEthnicity.setId(ethnicityId);
        existingEthnicity.setEthnicityName("Old Ethnicity");
    ...
    @Test
    void testUpdateEthnicityWithNullDTO() {
        EthnicityDTO ethnicityDTO = null;
        Long id = 1L;

        Mockito.when(ethnicityRepository.findById(id)).thenReturn(Optional.empty());

        ApiRequestException exception = Assertions.assertThrows(ApiRequestException.class, () -> ethnicityService.updateEthnicity(id, ethnicityDTO));
        Assertions.assertEquals("Ethnicity not found", exception.getMessage());
    }

    @Test
    void testUpdateEthnicityWithNullEthnicityName() {
        EthnicityDTO ethnicityDTO = new EthnicityDTO();
        ethnicityDTO.setEthnicityName(null);
        Long id = 1L;
        ...
        ApiRequestException exception = Assertions.assertThrows(ApiRequestException.class, () -> ethnicityService.updateEthnicity(id, ethnicityDTO));
        Assertions.assertEquals("Invalid ethnicity data", exception.getMessage());
    }

    @Test
    void testUpdateEthnicityWithEmptyEthnicityName() {
        EthnicityDTO ethnicityDTO = new EthnicityDTO();
        ethnicityDTO.setEthnicityName("");
        Long id = 1L;
    ...
    @Test
    void testUpdateEthnicityWithValidData() {
        Long id = 1L;
        String newEthnicityName = "New Ethnicity";
        ...
        Ethnicity existingEthnicity = new Ethnicity();
        existingEthnicity.setId(id);
        existingEthnicity.setEthnicityName("Old Ethnicity");

        when(ethnicityRepository.findById(id)).thenReturn(Optional.of(existingEthnicity));
        when(ethnicityRepository.existsByEthnicityName(newEthnicityName)).thenReturn(false);

        Ethnicity updatedEthnicity = new Ethnicity();
        updatedEthnicity.setId(id);
        updatedEthnicity.setEthnicityName(newEthnicityName);

        when(ethnicityRepository.save(any(Ethnicity.class))).thenReturn(updatedEthnicity);

        EthnicityDTO result = ethnicityService.updateEthnicity(id, ethnicityDTO);
    ...
    @Test
    void testUpdateEthnicityWithSameName() {
        Long id = 1L;
        String existingName = "Existing Ethnicity";

        EthnicityDTO ethnicityDTO = new EthnicityDTO();
        ethnicityDTO.setEthnicityName(existingName);

        Ethnicity existingEthnicity = new Ethnicity();
        existingEthnicity.setId(id);
        existingEthnicity.setEthnicityName(existingName);
        ...
        verify(ethnicityRepository, times(1)).findById(id);
        verify(ethnicityRepository, never()).existsByEthnicityName(existingName);
        verify(ethnicityRepository, never()).save(any(Ethnicity.class));
    }

    @Test
    void testUpdateEthnicityWithExistingName() {
        Long id = 1L;
        String newEthnicityName = "New Ethnicity";
        ...
        Ethnicity existingEthnicity = new Ethnicity();
        existingEthnicity.setId(2L);
        existingEthnicity.setEthnicityName("Existing Ethnicity");

        when(ethnicityRepository.findById(id)).thenReturn(Optional.of(existingEthnicity));
        when(ethnicityRepository.existsByEthnicityName(newEthnicityName)).thenReturn(true);

        assertThrows(ApiRequestException.class, () -> ethnicityService.updateEthnicity(id, ethnicityDTO));
